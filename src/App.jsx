...
export default function App() {
  ...
  // 39-Hour Guarantee pay (only if hours entered)
  const is39Elig = hasHours && check39 &&
    (role==="Driver"||role==="Trainer") &&
    rating==="Perfect" &&
    parseInt(days39||"0",10)>=3 &&
    dRejects==="No";
  const missingH     = is39Elig && totalH<39 ? 39-totalH : 0;
  const guaranteePay = hasHours? (base*missingH).toFixed(2) : "0.00";

  // Lunch Bonus (½ hr/day, only if hours entered)
  const lunchAmt = hasHours && checkLunch &&
    (role==="Driver"||role==="Trainer") &&
    rating==="Perfect" &&
    ["A","B"].includes(tier)
    ...
    <div className="p-8 max-w-3xl mx-auto font-sans space-y-8">
      ...
      <div className="bg-white p-6 rounded-lg shadow space-y-6">
        ...
        <div>
          ...
          >
            ...
            <option>&lt;1</option><option>1</option><option>2</option>
      ...
      <div className="bg-white p-6 rounded-lg shadow space-y-4">
        ...
        <ul className="list-disc ml-6 space-y-1">
          ...
          {(role==="Driver"||role==="Trainer") && check39 && hasHours && (
            <li>39-Hour Guarantee Pay: ${guaranteePay}</li>
          )}

          {(role==="Driver"||role==="Trainer") && checkLunch && hasHours && (
            <li>Lunch Bonus Total: ${lunchAmt}</li>
          )}

          <li>Hourly Bonus Total Pay (Max 40 hrs): ${hourlyBonusTotal}</li>
